...
public abstract class myCursorTreeAdapter extends ResourceCursorTreeAdapter {
     ...
     */
    private SimpleCursorTreeAdapter.ViewBinder mViewBinder;
     ...
     *            given the values of the first N columns in the from parameter.
     */
    public myCursorTreeAdapter(Context context, Cursor cursor, int collapsedGroupLayout,
     ...
     *            given the values of the first N columns in the from parameter.
     */
    public myCursorTreeAdapter(Context context, Cursor cursor, int groupLayout,
    ...
    /**
     * Returns the {@link SimpleCursorTreeAdapter.ViewBinder} used to bind data to views.
     *
    ...
    private void bindView(View view, Context context, Cursor cursor, int[] from, int[] to) {
        final SimpleCursorTreeAdapter.ViewBinder binder = mViewBinder;
    ...
    private void groupBindView(View view, Context context, Cursor cursor, int[] from, int[] to) {
        final SimpleCursorTreeAdapter.ViewBinder binder = mViewBinder;
        ...
        for (int i = 0; i < to.length; i++) {
            ...
            if (v != null) {
                ...
                if (!bound) {
                    ...
                    if (i == 0)
                        text = sdf.format(Long.parseLong(text));
                    if (v instanceof TextView) {
                        setViewText((TextView) v, text);
                    } else if (v instanceof ImageView) {
                        setViewImage((ImageView) v, text);
    ...
    protected void bindGroupView(View view, Context context, Cursor cursor, boolean isExpanded) {
        ...
        groupBindView(view, context, cursor, mGroupFrom, mGroupTo);
    }

    /**
     * Called by groupBindView() to set the image for an ImageView. By default, the
